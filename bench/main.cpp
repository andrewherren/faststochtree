...
// Simple CLI parser: accepts --key=value and --key value
...
int main(int argc, char** argv) {
    ...
    std::vector<double> X(n_total * p), y_all(n_total);
    for (int i = 0; i < n_total; i++) {
        for (int j = 0; j < p; j++) X[i * p + j] = data_rng.uniform();
        double signal = std::sin(2.0 * M_PI * X[i * p + 0]);
        y_all[i] = signal + sigma_true * data_rng.normal();
    }

    // tau: leaf prior variance — calibrated so ensemble prior std ≈ 0.5
    double tau = (0.5 * 0.5) / num_trees;

    bart::BARTConfig cfg;
    cfg.num_trees      = num_trees;
    cfg.leaf_prior_var = tau;
    cfg.sigma2_shape   = 3.0;
    cfg.sigma2_scale   = sigma_true * sigma_true;  // rough centering

    bart::RNG model_rng(seed + 1);

    printf("faststochtree BART benchmark (v1-naive-cpp)\n");
    ...
    printf("Running %d burnin iterations + %d samples...", n_burnin, n_samples); fflush(stdout);

    // Run BART: train on first n_train rows, predict on the remaining n_test rows
    const double* X_test = X.data() + n_train * p;
    ...
    for (double s2 : result.sigma2_samples) sigma2_post += s2;
