...
Format auto-detection:
    Single-run CSV (bench_results.csv):     one row per run; uses min time_ms.
    Multi-iteration CSV (bench_results_mc.csv): one row per iteration; uses
        mean time_ms and mean rmse across iterations.

Usage:
    python bench/plot_results.py                         # single-run CSV
    python bench/plot_results.py --mc                    # MC CSV (averaged)
...
DEFAULT_CSV_MC        = SCRIPT_DIR / "results" / "bench_results_mc.csv"
...
# ---------------------------------------------------------------------------
# Parse CSV — auto-detects single-run vs multi-iteration (MC) format.
#
# Single-run (bench_results.csv):     keeps the minimum time_ms per key
#                                     and the rmse from that same row.
# Multi-iteration (bench_results_mc.csv): averages time_ms and rmse
#                                     across all iterations for each key.
# ---------------------------------------------------------------------------

def load_data(csv_path):
    """Return (times, rmses) dicts keyed by (tag, n_train, p).

    For single-run CSVs: time = min, rmse = from that row.
    For MC CSVs (detected by 'iter' column): time = mean, rmse = mean.
    """
    with open(csv_path) as f:
        reader = csv.DictReader(f)
        fieldnames = reader.fieldnames or []
        is_mc = "iter" in fieldnames
        ...
        if is_mc:
            ...
            for row in reader:
                ...
                sum_time[key] += float(row["time_ms"])
                sum_rmse[key] += float(row["rmse"])
                counts[key]   += 1
            times = {k: sum_time[k] / counts[k] for k in counts}
            rmses = {k: sum_rmse[k] / counts[k] for k in counts}
        else:
            best_time: dict = defaultdict(lambda: float("inf"))
            best_rmse: dict = {}
            for row in reader:
                key = (row["tag"], int(row["n_train"]), int(row["p"]))
                t = float(row["time_ms"])
                if t < best_time[key]:
                    best_time[key] = t
                    best_rmse[key] = float(row["rmse"])
            times = dict(best_time)
            rmses = best_rmse
...
def plot_rmse_track(ax, labels, rmses, base_color, title, sigma_true, plt, ticker):
    ...
    for bar, r in zip(bars, rmses):
        if r is None:
            ...
            continue
        ax.text(bar.get_x() + bar.get_width() / 2,
                r + (ax.get_ylim()[1] - ax.get_ylim()[0]) * 0.01,
                f"{r:.4f}", ha="center", va="bottom", fontsize=8.5, rotation=45)
    ...
    # Tight y-axis with a little padding around the data range
    if valid:
        lo = min(min(valid), sigma_true)
        hi = max(max(valid), sigma_true)
        pad = (hi - lo) * 0.5 or 0.05
        ax.set_ylim(lo - pad, hi + pad * 2.5)   # extra top room for rotated labels


def make_figures(best_times, best_rmses, scenario, out_bart, out_gfr,
    ...
    plot_track(ax_bart, bart_labels, times_for(BART_VERSIONS), "#4C72B0",
    ...
    plot_rmse_track(ax_rmse_bart, bart_labels, rmses_for(BART_VERSIONS), "#4C72B0",
...
def main():
    ...
    ap.add_argument("--mc",            action="store_true",            help="use bench_results_mc.csv (averaged across iterations)")
    ...
    csv_path = args.csv or (DEFAULT_CSV_MC if args.mc else DEFAULT_CSV)
    times, rmses = load_data(csv_path)
    make_figures(times, rmses, (args.n, args.p),
